            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-lg-6">                    
                        ...
                        <div class="form-group">
                            <label>請假起日期時間</label>
                             <input class="form-control datepicker" id="vsday" type="text" placeholder="" value="{{ $info[0]['vsday'] }}">
                        </div>
                        <div class="form-group">
                            <label>請假天數</label>
                             <input class="form-control datepicker" id="sumday" type="text" placeholder="" value="{{ $info[0]['sumday'] }}">
                        </div>
                        <div class="form-group">
                            <label>請假事由</label>                            
                            <textarea class="form-control" id="reason" rows="3" >{{ $info[0]['reason'] }}</textarea>
                        </div>                                               
                    </div>
                    <!-- /.col-lg-6 (nested) -->
                    <div class="col-lg-6">
                        ...
                        <div class="form-group">
                            <label>請假迄日期時間</label>
                             <input class="form-control datepicker" id="veday" type="text" placeholder="" value="{{ $info[0]['veday'] }}">
                        </div>
                        <div class="form-group">
                            <label>請假時數</label>
                             <input class="form-control datepicker" id="sumhour" type="text" placeholder="" value="{{ $info[0]['sumhour'] }}">
                        </div>
                        <div class="form-group">
                            <label>備註</label>
                            <textarea class="form-control" id="memo" rows="3" >{{ $info[0]['memo'] }}</textarea>
                        </div>                                               
                    </div>
                    <input type="hidden" id="v_id" name="v_id" value="{{ $info[0]['id'] }}" />  
                    <!-- /.col-lg-6 (nested) -->
                </div>
                <!-- /.row (nested) -->
                <div class="row">
...
<script>
      $('#btn_confirm').click(function() {
		...
		$.ajax({
        	url: "{{ url('/backstage/vacation/edit') }}", // point to server-side PHP script 
